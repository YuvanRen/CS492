...
void* fs_init(struct fuse_conn_info *conn)
{
	...
	struct fs_super *sb = malloc(sizeof(struct fs_super));
/* Read superblock */
    if (disk->ops->read(disk, 0, 1, sb) != SUCCESS) {
        fprintf(stderr, "Failed to read the superblock.\n");
        free(sb);
        exit(1);
    }
    ...
    if (!inode_map) {
        fprintf(stderr, "Failed to allocate memory for inode map.\n");
        free(sb);
        exit(1);
    }
/* Read inode map */
    if (disk->ops->read(disk, inode_map_base, sb->inode_map_sz, inode_map) != SUCCESS) {
        fprintf(stderr, "Failed to read inode map.\n");
        free(inode_map);
        free(sb);
        exit(1);
	}
    ...
    if (!block_map) {
        fprintf(stderr, "Failed to allocate memory for block map.\n");
        free(inode_map);
        free(sb);
        exit(EXIT_FAILURE);
    }
/* Read block map */
    if (disk->ops->read(disk, block_map_base, sb->block_map_sz, block_map) != SUCCESS) {
        fprintf(stderr, "Failed to read block map.\n");
        free(block_map);
        free(inode_map);
        free(sb);
        exit(1);
...
static int fs_mkdir(const char *path, mode_t mode)
{
 ...
 /*Cannot recreate root.*/
    ...
    if (slash) {
        strncpy(new_dir_name, slash + 1, sizeof(new_dir_name) - 1);
        new_dir_name[sizeof(new_dir_name) - 1] = '\0'; 
    ...
    int existing_inum = lookup(parent_inum, new_dir_name);
    if (existing_inum != -ENOENT) {
        return existing_inum > 0 ? -EEXIST : existing_inum;
    }

    /* Load the directory entries of the parent directory */
    struct fs_dirent entries[DIRENTS_PER_BLK];
    if (disk->ops->read(disk, parent_inode->direct[0], 1, entries) < 0) {
        exit(1);
    }
    ...
    mode |= S_IFDIR; 
    int status = set_attributes_and_update(entries, new_dir_name, mode, true);
    if (status != SUCCESS) {
        return status;
    }

    /* Write the updated entries back to the parent directory block */ 
    if (disk->ops->write(disk, parent_inode->direct[0], 1, entries) < 0) {
        exit(1);
    }
...
static int fs_rmdir(const char *path)
{
	...
	char name[FS_FILENAME_SIZE];
	int parent_inode_idx = translate_1(_path, name);
	int inode_idx = lookup(parent_inode_idx, name);
	struct fs_inode *inode = &inodes[inode_idx];
	struct fs_inode *parent_inode = &inodes[parent_inode_idx];
	if (inode_idx < 0 || parent_inode_idx < 0) return -ENOENT;
    ...
    if (disk->ops->read(disk, inodes[parent_inode_idx].direct[0], 1, parent_entries) < 0) {
        return -EIO; 
    }
    ...
    for (int i = 0; i < DIRENTS_PER_BLK; i++) {
        if (parent_entries[i].valid && strcmp(parent_entries[i].name, name) == 0) {
            memset(&parent_entries[i], 0, sizeof(struct fs_dirent)); // Clear the directory entry
            found = true;
            break;
        }
    }
    ...
    if (disk->ops->write(disk, inodes[parent_inode_idx].direct[0], 1, parent_entries) < 0) {
        exit(1); 
    }
...
int fs_utime(const char *path, struct utimbuf *ut)
{
	//CS492: your code here
	char *_path = strdup(path);
    if (!_path) return -EINVAL; 
    int inode_idx = translate(_path); /* Returns errors */
    ...
    /* Update the inode times thorugh reference */ 
    inode->ctime = ut->modtime; /*  Modification time */
		    ...
		    struct fuse_file_info *fi)
{
    ...
    while (len > 0 && bytes_read < inode->size) {
        char block_buf[BLOCK_SIZE];
        if (disk->ops->read(disk, inode->direct[block_num], 1, block_buf) < 0) {
            return -EIO;  /* I/O error during block read */
        }
        ...
        len -= bytes_to_copy;
        block_offset = 0;  /* Only the first block may have an offset */
        block_num++;
    }
